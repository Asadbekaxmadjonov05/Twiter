...
function Home() {
  ...
  function handleSubmitPost(e) {
    ...
    const name = token?.login || 'Anonymous'; // Provide default value if `token` or `login` is undefined
    const imgUrl = Avatar || 'defaultAvatarUrl'; // Provide a default avatar URL if `Avatar` is undefined
    const postImg = postImg || 'defaultPostImgUrl'; // Provide a default post image URL if `postImg` is undefined
     const description = e.target.postValue?.value || '';
    const data = {
        id: updatedPostList.length ? updatedPostList[updatedPostList.length - 1].id + 1 : 1,
        name,
        imgUrl,
        email: "@inner · 25m",
        description,
    ...
    setPostList([data, ...updatedPostList]);
    e.target.reset();
}
